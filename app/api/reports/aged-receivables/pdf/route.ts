...
export async function GET(request: Request) {
  try {
    ...
    // Aggregate
    interface AgedEntry { clientName: string; loanNumber: string; totalDue: number; current: number; d1_30: number; d31_60: number; d61_90: number; d90plus: number; maxDaysOverdue: number }
    ...
    for (const s of schedules) {
      ...
      if (!reportData[s.loan_id]) {
        reportData[s.loan_id] = { clientName: `${loan.client?.first_name || ''} ${loan.client?.last_name || ''}`.trim(), loanNumber: loan.loan_number || 'N/A', totalDue: 0, current: 0, d1_30: 0, d31_60: 0, d61_90: 0, d90plus: 0, maxDaysOverdue: 0 }
      }
      const e = reportData[s.loan_id]
      e.totalDue += debt
      if (daysOverdue <= 0) e.current += debt
      else {
        if (daysOverdue > e.maxDaysOverdue) e.maxDaysOverdue = daysOverdue
        if (daysOverdue <= 30) e.d1_30 += debt
    ...
    for (const e of entries) {
      ...
      const diasCell = e.maxDaysOverdue > 0
        ? `<td style="text-align:center;color:${e.maxDaysOverdue > 90 ? '#dc2626' : e.maxDaysOverdue > 60 ? '#ea580c' : e.maxDaysOverdue > 30 ? '#ca8a04' : '#16a34a'};font-weight:700;">${e.maxDaysOverdue} días</td>`
        : `<td style="text-align:center;color:#64748b;">Al día</td>`
      rows += `<tr><td>${e.clientName}</td><td>${e.loanNumber}</td>${diasCell}<td class="currency">${fmtQ(e.totalDue)}</td><td class="currency">${fmtQ(e.current)}</td><td class="currency">${fmtQ(e.d1_30)}</td><td class="currency">${fmtQ(e.d31_60)}</td><td class="currency">${fmtQ(e.d61_90)}</td><td class="currency">${fmtQ(e.d90plus)}</td></tr>`
    }
    rows += `<tr style="font-weight:700;background:#f1f5f9;"><td colspan="3">TOTALES</td><td class="currency">${fmtQ(totals.totalDue)}</td><td class="currency">${fmtQ(totals.current)}</td><td class="currency">${fmtQ(totals.d1_30)}</td><td class="currency">${fmtQ(totals.d31_60)}</td><td class="currency">${fmtQ(totals.d61_90)}</td><td class="currency">${fmtQ(totals.d90plus)}</td></tr>`
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${reportCSS}</style></head><body>
      ...
      <table><thead><tr><th>Cliente</th><th>Préstamo</th><th>Días en Mora</th><th>Saldo Total</th><th>Corriente</th><th>1-30 Días</th><th>31-60 Días</th><th>61-90 Días</th><th>+90 Días</th></tr></thead>
