...
export async function GET(request: Request) {
  try {
    ...
    payments?.forEach((p: any) => {
      ...
      ;[6,13].forEach((idx) => { const c = row.getCell(idx); c.numFmt = dateFmt })
      ;[5,8,9,10,12].forEach((idx) => { const c = row.getCell(idx); c.numFmt = currencyFmt })
    })
    ...
    ws.mergeCells(summaryHeader.number, 1, summaryHeader.number, 14)
    ...
    ws.columns = [
      ...
      { width: 18 }, { width: 12 }, { width: 18 }, { width: 25 },
    ]
