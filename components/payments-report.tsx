...
export function PaymentsReport() {
  ...
  const generateExcelReport = async () => {
    ...
    try {
      ...
      reportData.clients.forEach(client => {
        client.payments.forEach((payment, index) => {
          excelData.push([
            ...
            payment.capital,
            typeof payment.interest === 'number' ? payment.interest : Math.max(0, (Number(payment.scheduledAmount || 0) - Number(payment.capital || 0) - Number(payment.mora || 0) - Number(payment.adminFees || 0))),
            payment.paidAmount,
            payment.paymentStatus,
            payment.mora,
            Number(payment.adminFees || 0),
      ...
      excelData.push([]) // Empty row
      // Compute total interests from data
      const totalIntereses = reportData.clients.reduce((sum, c) => sum + c.payments.reduce((s, p) => s + (typeof p.interest === 'number' ? p.interest : Math.max(0, (Number(p.scheduledAmount || 0) - Number(p.capital || 0) - Number(p.mora || 0) - Number(p.adminFees || 0)))), 0), 0)
      ...
      for (let row = 6; row < summaryHeaderRowIndex; row++) {
        ...
        for (let col = 0; col <= 15; col++) {
          const cellRef = XLSX.utils.encode_cell({ r: row, c: col })
          if (worksheet[cellRef]) {
            // Currency columns: 4, 7, 8, 9, 10, 12, 13
            if ([4, 7, 8, 9, 10, 12, 13].includes(col)) {
              worksheet[cellRef].s = isAlternate ? styles.currencyCellAlternate : styles.currencyCell
            }
            // Date columns: 5, 14 (Fecha de Pago, Fecha de Vencimiento)
            else if ([5, 14].includes(col)) {
      ...
      for (let row = summaryHeaderRowIndex; row <= range.e.r; row++) {
        ...
        if (worksheet[cellA] && worksheet[cellA].v === "RESUMEN EJECUTIVO") {
          ...
          // Style summary rows
           for (let summaryRow = row + 2; summaryRow <= row + 8; summaryRow++) {
             ...
             if (worksheet[valueCell]) {
               ...
               if ([row + 4, row + 5, row + 6, row + 7, row + 8].includes(summaryRow)) {
      ...
      for (let row = 0; row <= range.e.r; row++) {
        const cellA = XLSX.utils.encode_cell({ r: row, c: 0 })
        if (worksheet[cellA] && worksheet[cellA].v === "RESUMEN EJECUTIVO") {
          worksheet['!merges'].push({ s: { r: row, c: 0 }, e: { r: row, c: 15 } })
          break
        }
      }
